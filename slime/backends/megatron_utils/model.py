...
# Cheap "is the per-microbatch probe off?" test, evaluated once at import. The probe itself
# owns the real parsing (see slime_plugins/bpm/backend/mb_timing.py); this flag exists only so
# that the default path never imports it and never pays for it.
_MB_TIMING_REQUESTED = os.environ.get("BPM_MB_TIMING", "").strip().lower() not in ("", "0", "off", "false", "no")


def _get_microbatch_timer():
    """Return the per-microbatch wall-clock probe, or ``None`` when it is switched off.

    Off by default. See ``slime_plugins/bpm/backend/mb_timing.py`` for what it measures and
    why; it is a diagnostic for "is a slow step a few slow microbatches or all of them", and
    it synchronizes the device, so it is not meant to be left on.
    """
    if not _MB_TIMING_REQUESTED:
        return None
    from slime_plugins.bpm.backend.mb_timing import get_microbatch_timer

    return get_microbatch_timer()


def _wrap_forward_step_with_microbatch_pbar(forward_step_func, pbar):
    mb_timer = _get_microbatch_timer()
    if pbar is None and mb_timer is None:
        return forward_step_func

    def wrapped_forward_step(*args, **kwargs):
        if mb_timer is not None:
            mb_timer.boundary()
        result = forward_step_func(*args, **kwargs)
        if mb_timer is not None:
            mb_timer.mark_forward_end(result)
        if pbar is not None:
            model = args[1] if len(args) > 1 else kwargs.get("model")
            if model is not None and _should_update_microbatch_pbar(model):
                pbar.update(1)
        if mb_timer is not None:
            # After the progress bar, so its cost is charged to `bwd+` and not to the loss.
            result = mb_timer.wrap_loss(result)
        return result
...
def forward_only(
    ...
    mb_timer = _get_microbatch_timer()
    forward_tag = f"{(store_prefix or getattr(model[0], 'role', 'actor')).rstrip('_')}-forward"
    for step_id in range(num_steps_per_rollout):
        if mb_timer is not None:
            mb_timer.begin_step(forward_tag, None, step_id, num_microbatches[step_id])
...
def train_one_step(
    ...
    mb_timer = _get_microbatch_timer()
    if mb_timer is not None:
        mb_timer.begin_step(f"{getattr(model[0], 'role', 'actor')}-train", rollout_id, step_id, num_microbatches)
    ...
    if mpu.is_pipeline_last_stage(ignore_virtual=True):
        ...
        if getattr(args, "opd_mode", "off") != "off":
            from slime_plugins.bpm.backend.metric_reducer import reduce_bpm_metrics

            reduce_bpm_metrics(args, loss_reduced)
        return loss_reduced, grad_norm
    return {}, grad_norm


def _bpm_extra_batch_keys(args: Namespace) -> list[str]:
    """Extra per-sample batch keys the BPM loss path consumes from the rollout batch.

    These teacher-side fields (token ids the teacher prefilled, the teacher hidden states, and the
    optional top-k teacher log-probs) are populated by the teacher rollout stack and must be
    forwarded through ``get_batch`` so the BPM loss can read them. They flow
    through the generic pass-through path in ``get_batch`` (per-sample lists, sliced by the micro-batch
    schedule). When BPM is off (``opd_mode == "off"``, the default) this returns an empty list, so the
    batch key set is byte-for-byte the upstream one.
    """
    if getattr(args, "opd_mode", "off") == "off":
        return []
    keys = ["teacher_token_ids", "teacher_hidden_states", "teacher_topk_log_probs"]
    if getattr(args, "opd_backend", "bpm") == "gold":
        # GOLD aligns by byte offsets over the teacher's own response text.
        keys.append("teacher_response_text")
    return keys

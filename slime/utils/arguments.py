    ...
    def add_slime_arguments(parser):
        ...
        def add_rollout_arguments(parser):
            ...
            parser.add_argument(
                ...
                type=float,
                default=0.0,
                help="the frequency penalty for the inference engine during rollout.",
            )
            parser.add_argument(
        ...
        def add_rollout_buffer_arguments(parser):
            ...
            parser.add_argument(
                ...
                type=str,
                default="qwen",
                choices=["qwen", "qwen3", "qwen3_5", "gemma4", "distill_qwen", "distill_qwen_think"],
                help="Loss mask type",
            )
            parser.add_argument(
...
def _bpm_opt_in():
    """Return True only when the caller explicitly opts into the BPM distillation plugin.

    ``--opd-mode`` is BPM's master switch (default ``off`` = no distillation), so we pre-parse
    it here. Stock slime users never pass it, so the BPM flag surface, its ``slime_plugins``
    import, and its validation stay entirely absent (no ``--help`` change, no behavior change)
    for every non-BPM invocation. This keeps the plugin opt-in rather than opt-out.
    """
    temp_parser = argparse.ArgumentParser(add_help=False, allow_abbrev=False)
    temp_parser.add_argument("--opd-mode", type=str, default="off")
    temp_args, _ = temp_parser.parse_known_args()
    return temp_args.opd_mode not in (None, "off")
...
def parse_args(add_custom_arguments=None):
    ...
    # Opt-in only: register the BPM (Byte-Prefix Marginalization) distillation flag surface
    # solely when the caller passes --opd-mode (non-off) and did not supply its own provider.
    # Non-BPM users get zero BPM flags, no slime_plugins import, and no BPM validation.
    enable_bpm = add_custom_arguments is None and _bpm_opt_in()
    if enable_bpm:
        from slime_plugins.bpm.args.bpm_arguments import add_bpm_arguments

        add_custom_arguments = add_bpm_arguments
    ...
    # BPM distillation flag validation + BPM-only default rewrites. Only imported/run when the
    # BPM surface was registered above, so a stock slime checkout without slime_plugins never
    # imports the plugin during parse_args.
    if enable_bpm:
        from slime_plugins.bpm.args.bpm_argument_validation import validate_bpm_args

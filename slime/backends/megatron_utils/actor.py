...
class MegatronTrainRayActor(TrainRayActor):
    ...
    def sleep(self) -> None:
        ...
        # BPM keeps the frozen teacher lm_head as a plain nn.Linear outside the
        # torch_memory_saver region, so pause() does not evict it. Drop it before offload so the
        # generic train-offload path does not pay a blocking D2H copy for a tensor that reloads
        # from the teacher checkpoint.
        if getattr(self.args, "_bpm_teacher_lm_head", None) is not None:
            logger.info("[OPD][train_sleep] dropping cached teacher lm_head; reloaded lazily on next BPM loss")
            self.args._bpm_teacher_lm_head = None
    ...
    def wake_up(self) -> None:
        ...
        # Move the cached teacher lm_head back to this rank's GPU (matches its placement in the
        # BPM loss). Use current_device(), not .cuda(), so TP>1 ranks do not all land on cuda:0.
        if getattr(self.args, "_bpm_teacher_lm_head", None) is not None:
            self.args._bpm_teacher_lm_head = self.args._bpm_teacher_lm_head.to(
            ...
            if self.args.compute_advantages_and_returns:
                ...
                can_reuse_log_probs_in_loss = (
                    ...
                    and (
                        ...
                        # BPM standalone distillation replaces the policy loss outright: the
                        # objective is computed in the BPM custom loss and never consumes old
                        # log-probs, so this prepass is a full extra forward for nothing.
                        # Reproduction contract: the reference runs skip it here, which is why
                        # they report no log-probs stage at all.
                        or getattr(self.args, "opd_mode", "off") == "standalone_loss"
                    )
                ...
                # Consumed by compute_advantages_and_returns(), which must synthesize the
                # CP-local zero KL that the skipped prepass would otherwise have supplied.
                self.args._skip_current_log_probs_prepass_active = can_reuse_log_probs_in_loss
                if (

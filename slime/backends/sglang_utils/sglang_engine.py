...
# Engine control endpoints are direct ip:port peers; never route them through
# an inherited HTTP proxy (a proxied call can turn into a spurious 504 on slow
# operations like release_memory_occupation).
...
class SGLangEngine(RayActor):
    ...
    def _make_request(self, endpoint: str, payload: dict | None = None):
        ...
        response = _direct_http.post(url, json=payload or {})
        try:
            response.raise_for_status()
    ...
    def flush_cache(self):
        ...
        for _ in range(60):
            try:
                response = _direct_http.get(f"http://{self.server_host}:{self.server_port}/flush_cache")
                if response.status_code == 200:
                    break
    ...
    def shutdown(self):
        ...
        if self.worker_type != "encoder" and self.node_rank == 0:
            ...
            if parse(sglang_router.__version__) <= parse("0.2.1"):
                ...
                try:
                    all_workers = _direct_http.get(f"http://{self.router_ip}:{self.router_port}/workers").json()["workers"]
    ...
    def pause_generation(self):
        if self.node_rank != 0:
            return
        response = _direct_http.post(f"http://{self.server_host}:{self.server_port}/pause_generation", json={})
        response.raise_for_status()
        return response
    ...
    def continue_generation(self):
        if self.node_rank != 0:
            return
        response = _direct_http.post(f"http://{self.server_host}:{self.server_port}/continue_generation", json={})
        response.raise_for_status()
        return response
    ...
    def stop_profile(self):
        if self.node_rank != 0:
            return
        response = _direct_http.post(f"http://{self.server_host}:{self.server_port}/stop_profile", json={})
        response.raise_for_status()
        return response

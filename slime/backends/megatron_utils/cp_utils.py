...
def get_logits_and_tokens_offset_with_cp(
    ...
    *,
    ...
    lo = prompt_length - logits_shift
    hi = total_length - logits_shift
    logits_0 = (max(chunk_0[0], lo), min(chunk_0[1], hi))
    logits_1 = (max(chunk_1[0], lo), min(chunk_1[1], hi))

    # when the sequence is empty, make an empty slice to continue the gradient flow.
    if logits_0[0] < logits_0[1]:
        token_0 = (logits_0[0] + logits_shift, logits_0[1] + logits_shift)
    else:
        logits_0 = (0, 0)
        token_0 = (0, 0)

    if logits_1[0] < logits_1[1]:
        token_1 = (logits_1[0] + logits_shift, logits_1[1] + logits_shift)
    else:
        logits_1 = (0, 0)
        token_1 = (0, 0)
...
def all_gather_with_cp(
    tensor: torch.Tensor, total_length: int, response_length: int, *, logits_shift: int = 1
) -> torch.Tensor:
    """
    ...
    prompt_length = total_length - response_length
    window_lo = prompt_length - logits_shift
    window_hi = total_length - logits_shift
    ...
    if chunk_0.shape[0] == 0 and chunk_1.shape[0] == 0:
        ...
        # only first chunk
        left = zero(logits_offset[0][0] - window_lo)
        right = zero(window_hi - logits_offset[0][1])
        full_tensor = torch.cat([left, chunk_0, right], dim=0)
    elif chunk_0.shape[0] == 0 and chunk_1.shape[0] != 0:
        # only second chunk
        left = zero(logits_offset[1][0] - window_lo)
        right = zero(window_hi - logits_offset[1][1])

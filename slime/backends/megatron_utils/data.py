...
# Keys already reported as non-numeric by log_rollout_data (warn once per key).
_non_numeric_keys_warned: set[str] = set()
    ...
    if mpu.get_tensor_model_parallel_rank() == 0 and mpu.is_pipeline_last_stage():
        ...
        for key, val in rollout_data.items():
            if key in [
                ...
                "micro_batch_indices",
                "source_names",
                # BPM teacher payloads: per-sample ndarrays / ragged nested lists (hidden states,
                # prefilled token ids, top-k teacher log-probs) that are not scalar-reducible metrics
                # and would break the concat/mean path below. They are consumed by the BPM loss, not
                # logged here. Absent unless BPM is enabled, so this is a no-op for non-BPM runs.
                "teacher_token_ids",
            ...
            if isinstance(val, (list, tuple)):
                ...
                if isinstance(val[0], torch.Tensor):
                    ...
                    try:
                        ...
                        # Non-numeric per-sample field (strings, None, nested
                        # lists): not a scalar metric, skip instead of failing.
                        if key not in _non_numeric_keys_warned:
                            _non_numeric_keys_warned.add(key)
                            logger.warning(f"log_rollout_data: skipping non-numeric field {key!r}")
                        continue
                log_dict[key] = (sum_value, count)

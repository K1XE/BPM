...
RAY_DEFAULT_ENV_VARS = {
    ...
    "RAY_USE_UVLOOP": "0",
    # Megatron requires this whenever tensor or context parallelism is on.
    "CUDA_DEVICE_MAX_CONNECTIONS": "1",
}


def add_default_ray_env_vars(env_vars: dict[str, str] | None = None) -> dict[str, str]:
    # Propagate the driver's Python path configuration explicitly: some ray
    # deployments replace (rather than merge) job-level env_vars when an actor
    # declares its own, which would otherwise let a stale PYTHONPATH or the
    # raylet's working directory shadow this repo inside actors.
    passthrough = {k: os.environ[k] for k in ("PYTHONPATH", "PYTHONSAFEPATH") if k in os.environ}
    return RAY_DEFAULT_ENV_VARS | passthrough | (env_vars or {})

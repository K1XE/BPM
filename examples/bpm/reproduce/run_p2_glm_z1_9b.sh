...
# Paper pair P2: GLM-Z1-9B -> Qwen3.5-2B
# See README.md
...
export BPM_TEACHER_MODEL_PATH="${BPM_TEACHER_MODEL_PATH:?path to GLM-Z1-9B HF checkpoint}"
export TEACHER_TP_SIZE="${TEACHER_TP_SIZE:-1}"
export TEACHER_DP_SIZE="${TEACHER_DP_SIZE:-8}"
export DATA_PATH="${DATA_PATH:?on-policy prompt mix (see README: data schema)}"
export BPM_BETA="${BPM_BETA:-0}"

case "${BPM_BETA}" in
   1|1.0) export BPM_RKL_LAMBDA="${BPM_RKL_LAMBDA:-0.1}" ;;
esac

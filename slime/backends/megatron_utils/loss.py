...
def get_rollout_top_p_logprob_kwargs(args: Namespace, batch: dict[str, Any]) -> dict[str, Any]:
    ...
    if top_p_token_ids is None or top_p_token_offsets is None:
        # sglang builds that predate `return_top_p_token_ids` custom_params
        # support cannot report the sampled top-p candidate sets. Fall back to
        # full-softmax log-probs instead of failing.
        global _top_p_renorm_warned
        if not _top_p_renorm_warned:
            logging.getLogger(__name__).warning(
                "rollout engine did not return top-p candidate token ids; "
                "computing log-probs over the full softmax (top-p renormalization disabled)."
            )
            _top_p_renorm_warned = True
        return {}
    return {
...
def compute_advantages_and_returns(args: Namespace, rollout_data: RolloutBatch) -> None:
    ...
    # BPM standalone distillation skips the old/current log-prob prepass (see
    # actor.py), so `log_probs` is absent here. KL is zero by construction in that
    # configuration (kl_coef == 0), so synthesize the CP-local zero tensors the
    # prepass would have supplied instead of dereferencing None below.
    if log_probs is None and values is None and getattr(args, "_skip_current_log_probs_prepass_active", False):
        zeros = []
        for total_len, response_len, loss_mask in zip(total_lengths, response_lengths, loss_masks, strict=False):
            if loss_mask.numel() != response_len:
                loss_mask = loss_mask[-response_len:]
            loss_mask = slice_log_prob_with_cp(loss_mask, int(total_len), int(response_len))
            zeros.append(torch.zeros_like(loss_mask, dtype=torch.float32, device=loss_mask.device))
        log_probs = zeros
...
def policy_loss_function(
    ...
    # BPM auxiliary distillation term (--opd-mode aux_loss): add coef * BPM_loss on top of the true
    # PG objective. aux_loss has no clean upstream hook -- it must fold into this same backward pass --
    # so it is ported in-place here behind the ``opd_mode`` guard. Under calculate_per_token_loss=True
    # both the PG loss and the BPM backend return token-summed scalars in the same normalization space,
    # so coef == 0 is exactly policy_loss and coef > 0 is a true PG + BPM auxiliary objective. Inert
    # when BPM is off (``opd_mode == "off"``, the default).
    opd_aux_loss = None
    if getattr(args, "opd_mode", "off") == "aux_loss":
        opd_aux_coef = 1.0
        if opd_aux_coef != 0.0:
            from slime_plugins.bpm.backend.losses import bpm_opd_loss_function

            # policy_loss_function only receives the per-sample-MEAN reducer; the BPM backend also needs
            # the per-token-SUM reducer. Reconstruct both from ``batch["loss_masks"]`` exactly as
            # ``loss_function`` and the standalone custom-loss shim do. This is NOT redundant with the
            # passed-in ``sum_of_sample_mean`` param: when ``--use-tis``/``--get-mismatch-metrics`` is on,
            # the local ``sum_of_sample_mean`` name is rebound above (TIS branch) to a reducer over
            # ``modified_response_masks``; rebuilding here keeps the aux term on the original pre-TIS
            # loss-mask reducer regardless of that rebind.
            ...
            opd_sum_of_sample = get_sum_of_sample_mean(
                total_lengths,
                response_lengths,
                batch["loss_masks"],
                batch.get("rollout_mask_sums"),
            ...
            opd_aux_loss, _opd_aux_log = bpm_opd_loss_function(
                args, batch, logits, opd_sum_of_sample_mean, opd_sum_of_sample
            )
            loss = loss + opd_aux_coef * opd_aux_loss
    ...
    if opd_aux_loss is not None:
        reported_loss["opd_aux_loss"] = opd_aux_loss.clone().detach()
...
def loss_function(
    ...
    # BPM standalone distillation (--opd-mode standalone_loss) computes its whole objective in the
    # custom-loss slot; route it through the EXISTING custom_loss case rather than adding a
    # bpm-specific match arm. An explicit --custom-loss-function-path still wins (user override);
    # otherwise default to the BPM adapter entry. When BPM is off (``opd_mode == "off"``, the
    # default) this block is inert and dispatch is exactly upstream.
    loss_type = args.loss_type
    custom_loss_function_path = args.custom_loss_function_path
    if getattr(args, "opd_mode", "off") == "standalone_loss":
        loss_type = "custom_loss"
        if custom_loss_function_path is None:
    ...
    match loss_type:
        ...
        case "custom_loss":
            func = load_function(custom_loss_function_path)
        case _:
    ...
    # int(): Megatron accumulates num_tokens on an int, so a float tensor raises
    _ps_norm = getattr(args, "_opd_per_sample_normalizer", None)
    if _ps_norm is not None:
        num_tokens = num_tokens.new_full(num_tokens.shape, int(round(float(_ps_norm))))
        args._opd_per_sample_normalizer = None

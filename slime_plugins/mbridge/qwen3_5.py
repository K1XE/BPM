...
class Qwen3_5Bridge(Qwen2MoEBridge):
    ...
    def _weight_name_mapping_mcore_to_hf(self, mcore_weights_name: str) -> list[str]:
        ...
        # Tied-embedding safety: the shared-weight direct mapping must apply even
        # when the installed mbridge base never invokes the
        # _adjust_mapping_for_shared_weights init hook (unpinned upstream detail).
        if not getattr(self, "_shared_weights_adjusted", False):
            self._adjust_mapping_for_shared_weights()
            self._shared_weights_adjusted = True

...
class RolloutManager:
    ...
    def __init__(self, args, pg):
        ...
        # BPM teacher stack (cross-tokenizer distillation): a prefill-only SGLang engine that
        # returns per-response-token hidden states. Placement/startup live in the plugin; the
        # manager only holds the teacher state and calls the entry points. No-op unless
        # --opd-mode selects an on-policy-distillation run. Runs after the health monitors
        # exist: teacher startup offloads the student engines, which pauses monitoring.
        self._teacher_engine_services = []
        self._teacher_engine_service = None
        self._teacher_is_asleep = False
        if getattr(self.args, "opd_mode", "off") != "off":
            from slime_plugins.bpm.teacher import bpm_teacher_rollout

            bpm_teacher_rollout.init_bpm_teacher_engines(self)
    ...
    def dispose(self):
        if self._teacher_engine_services:
            ...
            bpm_teacher_rollout.shutdown_teacher_engines(self)
    ...
    def generate(self, rollout_id):
        ...
        if self.args.debug_rollout_only:
            ...
            return
        # BPM: normalize rollout-sampled student tokens, then run the teacher prefill forward and
        # inject teacher hidden states / token ids into the samples before conversion.
        if self._teacher_engine_services:
            from slime_plugins.bpm.teacher import bpm_teacher_payload, bpm_teacher_rollout, bpm_teacher_tokens

            normalize_metrics = bpm_teacher_tokens.normalize_opd_student_tokens(self, data)
            if self.args.offload_rollout:
                self.offload()
            # Clear before the call so a failure can never re-log the previous rollout's timings.
            self._last_opd_teacher_perf = None
            try:
                bpm_teacher_payload.inject_teacher_hidden_states(self, data)
            except Exception:
                bpm_teacher_rollout.best_effort_teacher_sleep_after_error(
                    self, "teacher hidden-state injection failed"
                )
                raise
            # The teacher forward is a separate phase that finishes after _log_rollout_data
            # above, so its `perf/opd_teacher_*` breakdown needs its own log call. The dict is
            # host-side floats already computed inside the teacher forward (wall-clock deltas,
            # python sums, numpy .nbytes) -- reading it touches no engine and adds no device
            # sync, so it stays on by default and is safe after the teacher went back to sleep.
            teacher_perf = getattr(self, "_last_opd_teacher_perf", None)
            if teacher_perf or normalize_metrics:
                log_dict = dict(teacher_perf or {})
                log_dict.update(normalize_metrics or {})
                log_dict["rollout/step"] = compute_rollout_step(self.args, rollout_id)
                log_dict["rollout/rollout_id"] = rollout_id
                logger.info(f"teacher perf {rollout_id}: {log_dict}")
    ...
    def offload(self):
        # The BPM teacher path releases the engines itself (at startup and at
        # the end of each teacher forward); consume the marker instead of
        # releasing twice — a second release wedges torch_memory_saver in
        # stock sglang. Health monitoring still pauses either way.
        if getattr(self, "_bpm_prereleased", False):
            self._bpm_prereleased = False
    ...
    def _convert_samples_to_train_data(self, samples: list[Sample] | list[list[Sample]]):
        ...
        # The top-p candidate sets are only available from sglang builds that
        # support the `return_top_p_token_ids` custom_params; older engines
        # simply do not report them, in which case the loss falls back to
        # full-softmax log-probs (see megatron_utils/loss.py).
        ...
        # BPM teacher payload, injected by the teacher forward. teacher_hidden_states are numpy
        # arrays and teacher_token_ids are ragged per-sample lists; both are passed through the
        # tensorizer allowlist untouched and materialized lazily inside the loss.
        for _tkey in ("teacher_token_ids", "teacher_hidden_states", "teacher_response_text"):
            _tvals = [getattr(sample, _tkey, None) for sample in samples]
            if any(value is not None for value in _tvals):
                train_data[_tkey] = _tvals
...
def compute_metrics_from_samples(args, samples):
    ...
    log_dict |= _compute_domain_reward_metrics(args, samples)
...
def _compute_domain_reward_metrics(args, all_samples: list[Sample]):
    """Per-domain mean raw reward + counts (metadata.domain in {'math','code'}).

    Surfaces as rollout/raw_reward_math, rollout/raw_reward_code, rollout/n_math,
    rollout/n_code. The aggregate raw reward averages over every domain in the
    mix, which dilutes each; this splits it. Code rows without recoverable tests
    are ungradeable (reward 0 by construction) and excluded from the code mean,
    counted separately as n_code_ungradeable."""

    def _rv(s):
        v = s.get_reward_value(args)
        if isinstance(v, dict):  # defensive: --reward-key unset -> reward is the raw dict
            v = v.get("acc", next(iter(v.values()), 0.0))
        return float(v)

    math_r, code_r, code_ungrade = [], [], 0
    ...
    out = {}
    if math_r:
        out["raw_reward_math"] = float(np.mean(math_r))
        out["n_math"] = len(math_r)
    if code_r:
        out["raw_reward_code"] = float(np.mean(code_r))
        out["n_code"] = len(code_r)
    if code_ungrade:
        out["n_code_ungradeable"] = code_ungrade
    return out
